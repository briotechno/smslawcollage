...
const EditCalendarEventPage = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (res.ok && data?.success) {
        showToast({ type: "success", title: "Event updated", message: "Calendar event updated" });
        router.push("/admin/Calendar");
      } else {
        showToast({ type: "error", title: "Update failed", message: data.message || "Update failed" });
      }
    } catch (err) {
  ...
  return (
    <AdminLayout title="Edit Calendar Event">
      ...
      <form className="max-w-xl mx-auto bg-white shadow-lg rounded-lg p-8 flex flex-col gap-6" onSubmit={handleSubmit}>
        ...
        <div className="flex justify-end gap-3 mt-8">
          <button type="button" onClick={() => router.push('/admin/Calendar')} className="px-4 py-2 border border-gray-300 text-gray-700 rounded-md hover:bg-gray-50 transition-colors">Cancel</button>
          <button
            type="submit"
            className={`bg-purple-600 text-white px-6 py-3 rounded font-semibold text-lg flex items-center justify-center gap-2 ${loading ? "opacity-70 cursor-wait" : "hover:bg-purple-700"}`}
            disabled={loading}
          >
            {loading ? (
