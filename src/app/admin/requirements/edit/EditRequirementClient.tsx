    ...
    (async () => {
      ...
      try {
        ...
        if (raw) {
          ...
          if (pref.file) {
            ...
            try {
              setUploadedFileName(pref.file.split("/").pop());
            } catch {}
          }
          // clear prefetch after using it
          try {
            sessionStorage.removeItem(`prefetch:requirement:${id}`);
          } catch {}
      ...
      try {
        const res = await import("@/lib/adminFetch").then((m) =>
          m.default(`/api/requirements?id=${encodeURIComponent(id)}`)
        );
        ...
        if (res.ok && data.success && data.data) {
          ...
          if (data.data.file) {
            ...
            try {
              setUploadedFileName(data.data.file.split("/").pop());
            } catch {}
          }
        }
      } catch (err) {
                      ...
                      try {
                        ...
                        if (res.ok && data.success && data.url) {
                          ...
                          showToast({
                            type: "error",
                            title: "Upload failed",
                            message: data?.message || "Failed to upload PDF",
                          });
                        }
                      } catch (err) {
                        console.error(err);
