...
export async function GET(request: Request) {
  try {
    ...
    if (id) {
      const [rows] = (await db.execute("SELECT * FROM news WHERE id = ?", [id])) as any;
      await db.end();
    ...
    const [rows] = (await db.execute("SELECT * FROM news ORDER BY date DESC, id DESC")) as any;
    await db.end();
    (rows as any[]).forEach((row: any) => { if (row.tags) row.tags = JSON.parse(row.tags); });
...
export async function PUT(request: Request) {
  ...
  try {
    ...
    const db = await connectDB();
    await ensureNewsTable(db);
  const [result] = (await db.execute(`UPDATE news SET ${fields.join(", ")} WHERE id = ?`, params)) as any;
    await db.end();
...
export async function DELETE(request: Request) {
  ...
  try {
    ...
    const db = await connectDB();
    await ensureNewsTable(db);
  const [result] = (await db.execute("DELETE FROM news WHERE id = ?", [id])) as any;
    await db.end();
